...
const SearchBar: React.FC<SearchBarProps> = ({ results, setResults }) => {
  ...
  const fetchSuggestions = async (term: string) => {
    try {
      const response = await axios.get(`http://localhost:3000/api/companies?search=${term}`);
  ...
  const fetchResults = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/api/companies?search=${searchTerm}`);
  ...
  return (
    <div className="flex flex-col items-center mt-4">
      ...
      {/* <div className="mt-4 w-full sm:w-[33rem]">
        {(results || []).length > 0 && results.map((company) => (
          <div key={company.id} className="p-4 bg-white border border-gray-300 rounded-md mb-2 shadow-sm">
